...
const updatedContent = content.replace(regex, (match, name, url) => {
  ...
  // Using a=attention instead of a=face. 
  // Attention uses saliency detection which is often better for "Recognition" as it includes 
  // hair/shoulders/context that help identify the person. 
  // Added mod=1.05 and con=5 to make the person "pop"
  const smartUrl = `https://wsrv.nl/?url=${aggressiveEncode(originalUrl)}&w=1000&h=1000&fit=cover&a=attention&precrop=true&mod=1.05&con=5`;
  
  return `{ name: "${name}", imageUrl: "${smartUrl}" }`;
});

fs.writeFileSync(CELEBRITIES_FILE, updatedContent);
console.log('Switched to Attention-based smart cropping for better recognition.');
